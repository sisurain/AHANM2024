...
% with Bayesian Lasso and Horseshoe
clear; close all; clc;
...
for di = 1:p
    ...
    nu_hs{di} = 1/gamrnd(.5,1);
    xi_hs{di} = 1/gamrnd(.5,1);
    tau2_hs{di} = 1/gamrnd(.5,xi_hs{di});
    lambda2_hs{di} = 1/gamrnd(.5,nu_hs{di});
end
...
for isim = 1:nsim+burnin
    ...
    for j = 1:p
        ...
        % sample a and irho (Bayesian Lasso), and Horseshoe
        ...
        Ka{j} = 1/(lambda2_hs{j}*tau2_hs{j})+Xa{j}'*DD{j}*Xa{j};
        ahat{j} = Ka{j}\(Xa{j}'*D{j}'*(YY{j}-d{j}*tau{j}*D{j}*gam{j}));
        a{j+1} = ahat{j}+chol((1/sig2)*Ka{j},'lower')'\randn(1,1); 

        lambda2_hs{j} = 1/gamrnd(1, (nu_hs{j}*tau2_hs{j}*sig2)/(tau2_hs{j}*sig2+nu_hs{j}*a{j+1}^2/2));
        tau2_hs{j} = 1/gamrnd(1, (xi_hs{j}*lambda2_hs{j}*sig2)/(lambda2_hs{j}*sig2+xi_hs{j}*a{j+1}^2/2));
        nu_hs{j} = 1/gamrnd(1, lambda2_hs{j}/(1+lambda2_hs{j}));
        xi_hs{j} = 1/gamrnd(1,tau2_hs{j}/(1+tau2_hs{j}));
    end
