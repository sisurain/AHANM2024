...
rmse_values_mars <- numeric(n_iterations)
...
for (i in 1:n_iterations) {
  ...
  # Fit MARS model using the earth package
  mars_model <- earth(y_train ~ ., data = as.data.frame(X_train))
  ...
  # Predict on testing set using the MARS model
  predictions_mars <- predict(mars_model, newdata = as.data.frame(X_test))
  ...
  #rmse_values_mars[i] <- sqrt(mean((predictions_mars - y_test)^2))
  rmse_values_mars[i] <- sum((predictions_mars - y_test)^2)
}
...
mean_rmse_mars <- mean(rmse_values_mars)
sd_rmse_mars <- sd(rmse_values_mars)
...
cat("MARS - Mean RMSE over 100 iterations: ", mean_rmse_mars, "\n")
cat("MARS - Standard deviation of RMSE over 100 iterations: ", sd_rmse_mars, "\n")
