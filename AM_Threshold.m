...
th = 0; %X{4} is the nonlinear component with threshold

experiment = 4;
switch experiment
    case 1
        Y(X{4}>th) = Y(X{4}>th)+1*X{2}(X{4}>th)+3*X{3}(X{4}>th)+19*X{4}(X{4}>th)...
            +7*X{5}(X{4}>th)+9*X{6}(X{4}>th)+11*X{7}(X{4}>th)+13*X{8}(X{4}>th)...
            +15*X{9}(X{4}>th)+17*X{10}(X{4}>th)+19*X{11}(X{4}>th);
        Y(X{4}<=th) = Y(X{4}<=th)+1*X{2}(X{4}<=th)+3*X{3}(X{4}<=th)+5*X{4}(X{4}<=th)...
            +7*X{5}(X{4}<=th)+9*X{6}(X{4}<=th)+11*X{7}(X{4}<=th)+13*X{8}(X{4}<=th)...
            +15*X{9}(X{4}<=th)+17*X{10}(X{4}<=th)+19*X{11}(X{4}<=th);
...
subplot(2,2,2)
y3 = zeros(length(A{3}),1);
y3(A{3}>th) = 19*A{3}(A{3}>th);  
y3(A{3}<=th) = 5*A{3}(A{3}<=th); 
plot(A{3},theta_hat{3}-sum(theta_hat{3}-y3)/k{3},'bo','MarkerIndices',1:10:length(theta_hat{3}))
hold on
%plot(A{3}, 5*A{3},'r+','MarkerIndices',1:10:length(A{3}))
plot(A{3},y3,'r+','MarkerIndices',1:10:length(theta_hat{3}))
title('3rd Component')
...
plot(A{4}, 0*A{4},'r+','MarkerIndices',1:10:length(A{4}))
title('4th Component')
axis([0 1 -1 1])
